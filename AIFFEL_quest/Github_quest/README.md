...
# 노드 회고
 ...
 먼저 이번 프로젝트를 진행 하면서 마크다운과 github에 파일을 올리는 것에 익숙해 졌다.  
하지만 마크 다운의 문법 중에서 ~표시를 이용한 줄그음 때문에 날짜 표시가 뜻하지 않게  
줄그음 표시가 나는 것을 보게 되었고 이를 - 표시로 수정 하기는 했지만 정확한 ~표시  
쓰는 방법을 잘 모르겠다. 검색해서도 나오지 않는다.  
이 처럼 마크 다운은 유용하지만 문서에  뜻하지 않은 표현이 나올 가능성이 있어서  
그 점에 유의해서 문서를 작성해야 겠다는 생각이 든다.
 
 github에 대해서 배우면서 이제 나도 뭔가 전문적으로 배운다는 느낌이 들어서 좋았다.  
물론 아직 초보적인 단계이지만 시작이 반이라고 하니 마음이 뿌듯하다. 앞으로 관심을  
가지고 좀더 파일을 잘 관리 할 수 있게 배워나가야 겠다.
